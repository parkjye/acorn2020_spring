...
<script>
	...
	myApp.controller("msgCtrl", function($scope){
		//페이지가 로딩되는 시점에 msg라는 모델을 정의하고 배열 저장하기
		$scope.msgs=[{msg:"하나"},{msg:"두울"}];
		
		//페이지가 로딩되는 시점에 send라는 함수 정의하기
		$scope.send=function(){
			//msgs 배열에 새로운 object만들어서 저장하기(view에 자동 반영된다.)
		...
		$scope.getPhones=function(){
			...
			$scope.phones.push({name:"갤럭시 S5",snippet:"삼성에서 만든 방수폰 입니다.",price:110});
...
<body ng-app="myApp">
	<div class="container" ng-controller="msgCtrl">
		...
		<table>
			...
			<tr ng-repeat="tmp in phones | filter:keyword | orderBy:orderValue:isReverse"><!-- true == DESC -->
				<th ng-bind="tmp.name"></th>
				<th ng-bind="tmp.snippet"></th>
				<th>{{tmp.price}}</th>
			</tr>
		</table>
